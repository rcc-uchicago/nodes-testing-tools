#!/bin/bash
# extract the output of the mpgadget run to get the values for comparison
...
TPS=$(grep Step $filecpu | tail -n1 | sed 's/,//g' |  awk '{print(($10/$2)) }')
#
...
Powspec5=$(echo "$Powspec" | awk '{print$5}')
#
echo "---"
echo "  output:"
